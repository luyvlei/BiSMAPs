...
class GaussianMixture(torch.nn.Module):
    ...
    def _init_params(self):
        ...
        # train模式下每次动态计算方差矩阵的模以及精度矩阵，eval模式使用保存的结果
        self.train = True
    ...
    def fit(self, x, delta=1e-3, n_iter=100, warm_start=False):
        ...
        while (i <= n_iter) and (j >= delta):
            ...
            if torch.isinf(self.log_likelihood.abs()) or torch.isnan(self.log_likelihood):
                ...
                print("inf",self.mu.data.dtype,self.var.data.dtype)
            ...
            # print("cycle_cost = {}".format(time.time() - time_start))
    ...
    def _estimate_log_prob(self, x):
        ...
        if self.covariance_type == "full":
            ...
            if self.use_float32:
                mu = mu.float()
                var = var.float()
                log_2pi = d * np.log(2. * pi).astype(np.float32)
            else:
                x = x.double()
                mu = mu.double()
                log_2pi = d * np.log(2. * pi).astype(np.float64)
    ...
    def _m_step(self, x, log_resp):
        ...
        if self.covariance_type == "full":
            ...
            var = _cal_var_serial(x, mu, resp, batch_size=500)
        elif self.covariance_type == "diag":
...
if __name__=="__main__":
    ...
    data, label = make_blobs(n,256,centers=4,cluster_std=1.0)#make_moons(n_samples=n, shuffle=True, noise=0.03, random_state=None)
    ...
    mu_init = get_kmeans_mu(X, K)
    gmm = GaussianMixture(n_components=K, n_features=256, mu_init=mu_init,covariance_type="full",).cuda()
    ...
    ax.set_title("torch full covariance")

    gmm = GaussianMixture(n_components=K, n_features=256, mu_init=mu_init,covariance_type="diag", ).cuda()
    gmm.fit(X, n_iter=100)
    pre_label = gmm.predict(X)
    pre_label = pre_label.detach().cpu().numpy()
    ax = fig.add_subplot(2, 2, 3, projection='3d', facecolor='white')
    ax.scatter(data[:n1, 0], data[:n1, 1], c=pre_label[:n1])
    ax.set_title("torch diag covariance")

    from sklearn.mixture import GaussianMixture as GMM
    gmm =GMM(n_components=4,init_params='kmeans')
    gmm.fit(X.cpu().detach().numpy())
    pre_label = gmm.predict(X.cpu().detach().numpy())
